...
#include <sys/stat.h>
/* Generate a k-dimensional undirected grid, the size of each dimension is decided
by the input */

int main(int argc, char* argv[])
{
  // process the command line
  if (argc < 5) {fprintf(stderr, "USAGE: %s min_vertices max_vertices dims dim_size\n", argv[0]); exit(-1);}
  ...
  if (n >= minV && n <= maxV) {
    ...
    // save the graph
    printf("\nUndirected %d-dimensional grid\n", dim);
    char name3[256];
    sprintf(name3, "%s/undirect%ddim_grid_%dn_%de.egr", outpath, dim, n, m2 * 2);
