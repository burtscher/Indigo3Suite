...
static double CPPsssp_edge(const int src, const ECLgraph g, data_type* const dist, const int* const sp, const int threadCount);
...
{
  printf("sssp edge-based C (%s)\n", __FILE__);
  if (argc != 4) {fprintf(stderr, "USAGE: %s input_file_name source_node_number thread_count\n", argv[0]); exit(-1);}
  ...
  for (int i = 0; i < runs; i++) {
    runtimes[i] = CPPsssp_edge(source, g, distance, sp, threadCount);
  }
