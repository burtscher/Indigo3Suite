...
static double CPPsssp_vertex(const int src, const ECLgraph g, data_type* const dist, const int threadCount);
...
int main(int argc, char* argv[])
{
  ...
  for (int i = 0; i < runs; i++) {
    runtimes[i] = CPPsssp_vertex(source, g, distance, threadCount);
  }
